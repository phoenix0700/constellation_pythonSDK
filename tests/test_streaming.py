...
class TestNetworkEventStream:
    ...
    def setup_method(self):
        ...
        with patch(
            "constellation_sdk.streaming.Network", return_value=self.mock_network
        ):
    ...
    async def test_websocket_connection_failure(self, mock_websockets):
        ...
        with patch.object(
            self.stream, "_start_polling", new_callable=AsyncMock
...
class TestConvenienceFunctions:
    ...
    def test_create_event_stream(self):
        ...
        with patch(
            "constellation_sdk.streaming.NetworkEventStream"
    ...
    async def test_stream_balance_changes(self):
        ...
        with patch(
            ...
            with patch(
                "constellation_sdk.streaming.BalanceTracker"
                ...
                # Verify stream was configured correctly
                mock_stream.on.assert_called_once_with(
                    EventType.BALANCE_CHANGE, mock_callback
                )
                mock_stream.connect.assert_called_once()
...
@pytest.mark.skipif(
    not pytest.importorskip("websockets", reason="websockets not available"),
