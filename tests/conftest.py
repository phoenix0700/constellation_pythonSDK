...
from constellation_sdk import Account, MetagraphClient, Network, Transactions
...
def test_accounts(alice_account, bob_account):
    ...
    return {"alice": alice_account, "bob": bob_account, "charlie": Account()}
...
def mock_metagraph_responses():
    ...
    return {
        ...
        "metagraph_balance": {"data": {"balance": 0}},
    }
...
def signature_validator():
    ...
    def validate_signature_structure(signed_tx):
        ...
        return (
            "value" in signed_tx
            and "proofs" in signed_tx
            and isinstance(signed_tx["proofs"], list)
            and len(signed_tx["proofs"]) > 0
            and "id" in signed_tx["proofs"][0]
            and "signature" in signed_tx["proofs"][0]
        )
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "mock: mark test as using mock objects")
