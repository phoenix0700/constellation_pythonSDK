...
class TestBatchResponse:
    ...
    def test_batch_response_methods(self):
        ...
        # Test success_rate
        assert abs(response.success_rate() - 66.67) < 0.1  # 66.67% (allowing for floating point precision)
...
class TestBatchValidator:
    ...
    def test_validate_batch_operations(self):
        ...
        # Duplicate IDs
        operations = [
            BatchOperation(BatchOperationType.GET_BALANCE, {'address': 'DAG123...'}, id='dup'),
            BatchOperation(BatchOperationType.GET_BALANCE, {'address': 'DAG456...'}, id='dup'),
        ]
        errors = BatchValidator.validate_batch(operations)
        assert len(errors) == 1
...
class TestBatchOperationsIntegration:
    ...
    def test_real_batch_request_with_mock_network(self):
        ...
        with patch('constellation_sdk.network.requests.request') as mock_request:
            ...
            operations = [
                create_batch_operation('get_balance', {'address': 'DAG123...'}, 'balance1'),
                create_batch_operation('get_balance', {'address': 'DAG456...'}, 'balance2'),
            ]
