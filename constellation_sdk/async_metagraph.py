...
from .config import get_config, NetworkConfig, METAGRAPH_ENDPOINTS
from .exceptions import (
    MetagraphError, NetworkError, ValidationError, ConstellationError
)
...
class AsyncMetagraphClient:
    ...
    async def get_info(self) -> Dict[str, Any]:
        ...
        if self._is_cache_valid(cache_key):
            self.logger.logger.debug(f"Returning cached info for metagraph {self.metagraph_id}")
    ...
    async def get_balance(self, address: str) -> Dict[str, Any]:
        ...
        if self._is_cache_valid(cache_key):
            self.logger.logger.debug(f"Returning cached balance for {address} in {self.metagraph_id}")
            return self._cache[cache_key]
    ...
    async def get_latest_snapshot(self) -> Dict[str, Any]:
        ...
        if self._is_cache_valid(cache_key):
            self.logger.logger.debug(f"Returning cached snapshot for {self.metagraph_id}")
            return self._cache[cache_key]
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            self.logger.logger.info(f"Metagraph {self.metagraph_id} health check passed in {elapsed:.2f}s")
            return True
        
        except Exception as e:
            self.logger.logger.error(f"Metagraph {self.metagraph_id} health check failed: {e}")
            return False
...
class AsyncMetagraphDiscovery:
    ...
    async def discover_metagraphs(self, force_refresh: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    async def batch_health_check(
        self,
        metagraph_ids: Optional[List[str]] = None
    ) -> Dict[str, bool]:
        """
        ...
        if metagraph_ids is None:
            ...
            try:
                ...
                metagraph_ids = [mg.get("id", mg.get("metagraphId", str(i))) 
                               for i, mg in enumerate(discovered)]
            except Exception as e:
                self.logger.logger.error(f"Failed to discover metagraphs for health check: {e}")
                return {}
        ...
        async def check_metagraph(mg_id):
            try:
                ...
                self.logger.logger.warning(f"Health check failed for metagraph {mg_id}: {e}")
                return mg_id, False
...
async def batch_get_balances_from_multiple_metagraphs(
    address_metagraph_pairs: List[tuple],
    network_config: Optional[NetworkConfig] = None
    ...
    try:
        ...
        raise MetagraphError(f"Failed to get batch balances from multiple metagraphs: {e}") from e 