...
from . import (
    Account, Network, MetagraphClient, Transactions,
    get_config, set_config, create_custom_config,
    discover_production_metagraphs, ValidationError,
    ConstellationError, NetworkError, ASYNC_AVAILABLE
)
...
@click.group()
@click.option('--network', '-n', default=None, help='Network to use (mainnet, testnet, integrationnet)')
@click.option('--output', '-o', type=click.Choice(['pretty', 'json']), default=None, help='Output format')
...
def account_info(ctx, address):
    ...
    if not address:
        ...
        if not private_key:
            click.echo("❌ No address provided and no default private key configured", err=True)
            sys.exit(1)
...
@click.option('--watch', '-w', is_flag=True, help='Watch balance changes (refresh every 30s)')
@click.pass_context
@handle_errors
...
def send_transaction(ctx, amount, to_address, from_key, fee, dry_run):
    ...
    if not private_key_hex:
        click.echo("❌ No private key provided. Use --from-key or configure default key", err=True)
        sys.exit(1)
    ...
    if amount > current_balance:
        click.echo(f"❌ Insufficient balance. Available: {current_balance}, Requested: {amount}", err=True)
        sys.exit(1)
