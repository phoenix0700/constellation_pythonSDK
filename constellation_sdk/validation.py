...
from .exceptions import (
    ...
    MetagraphIdValidationError, TransactionValidationError, InvalidDataError
)
    ...
    def validate(cls, address: str, check_checksum: bool = False) -> None:
        ...
        if len(address) != 38:
            raise AddressValidationError(
                address, 
                f"Address must be exactly 38 characters, got {len(address)}"
            )
    ...
    def validate(cls, amount: Union[int, float], allow_zero: bool = False) -> None:
        ...
        if isinstance(amount, float):
            if amount != int(amount):
                raise AmountValidationError(amount, "Amount must be an integer (no decimals)")
            amount = int(amount)
        ...
        if amount < cls.MIN_AMOUNT and not (allow_zero and amount == 0):
            raise AmountValidationError(
                amount, 
                f"Amount must be at least {cls.MIN_AMOUNT}"
            )
        
        if amount > cls.MAX_AMOUNT:
            raise AmountValidationError(
                amount, 
                f"Amount cannot exceed {cls.MAX_AMOUNT}"
            )
...
class MetagraphIdValidator:
    ...
    def validate(cls, metagraph_id: str) -> None:
        ...
        if not isinstance(metagraph_id, str):
            raise MetagraphIdValidationError(metagraph_id, "Metagraph ID must be a string")
        
        if not metagraph_id:
            raise MetagraphIdValidationError(metagraph_id, "Metagraph ID cannot be empty")
        ...
        try:
            ...
            raise MetagraphIdValidationError(metagraph_id, f"Invalid format: {e.message}")
