...
class MetagraphClient:
    ...
    def discover_metagraphs(self, include_test_deployments: bool = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if response.status_code != 200:
                raise MetagraphError(f"Failed to fetch metagraphs: {response.status_code}")
            ...
            for currency in currencies:
                ...
                if not include_test_deployments and metagraph['category'] in ['test', 'automated']:
                    continue
        ...
        for network in networks:
            try:
                ...
                if response.status_code == 200:
                    ...
                    production_estimate = count if network == 'mainnet' else max(0, count - int(count * 0.9))
                    ...
                    summary['networks'][network] = {'total': 0, 'estimated_production': 0, 'likely_test_deployments': 0}
            except:
                summary['networks'][network] = {'total': 0, 'estimated_production': 0, 'likely_test_deployments': 0}
