...
def subscription_demo():
    ...
    tx_subscription = (SubscriptionBuilder()
                       .transaction_updates(addresses)
                       .build())
    ...
    balance_subscription = (SubscriptionBuilder()
                           .balance_updates(addresses)
                           .build())
    ...
    metagraph_subscription = (SubscriptionBuilder()
...
async def async_graphql_demo():
    ...
    try:
        ...
        network_response = await execute_query_async(
            'testnet',
            ConstellationSchema.NETWORK_STATUS
        )
    ...
    try:
        ...
        tasks = [get_account_data(addr) for addr in addresses[:2]]  # Limit to 2 for demo
    ...
    try:
        ...
        event_count = 0
        async for response in client.subscribe(subscription_query, {"addresses": addresses[:1]}):
            if response.is_successful:
                event_count += 1
                tx_data = response.data.get('transactionUpdates', {})
                print(f"  📝 Event {event_count}: Transaction {tx_data.get('hash', 'N/A')[:12]}...")
...
def convenience_functions_demo():
    ...
    print("\n• Account Query:")
    account_query = build_account_query(account.address, include_transactions=True, include_balances=True)
    print(f"  Length: {len(account_query)} characters")
    
    print("\n• Metagraph Query:")
    metagraph_query = build_metagraph_query(metagraph_id, include_holders=True, include_transactions=True)
...
def performance_demo():
    ...
    try:
        ...
        if response.is_successful:
            ...
            total_transactions = sum(len(acc.get('transactions', [])) for acc in accounts_data)
    ...
    print(f"   • Queries executed: {stats['queries_executed']}")
    print(f"   • Average execution time: {stats['average_execution_time']:.3f}s")
    print(f"   • Error rate: {stats['errors_encountered'] / max(stats['queries_executed'], 1) * 100:.1f}%")
