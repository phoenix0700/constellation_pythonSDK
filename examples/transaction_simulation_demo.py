...
def main():
    ...
    batch_transfers = [
        ...
        {"data": {"sensor": "humidity", "value": 65.2}, "metagraph_id": metagraph_id},
    ]

    batch_simulation = Transactions.simulate_batch_transfer(
        source=alice.address, transfers=batch_transfers, network=network
    )
    ...
    print(
        f"Batch Success Probability: {batch_simulation['batch_success_probability']:.1%}"
    )
    ...
    for i, result in enumerate(batch_simulation["individual_results"]):
        print(
            f"  Transfer {i+1}: {result['transaction_type']} - "
    ...
    print(
        f"Quick DAG simulation: {'✅ Success' if quick_sim['will_succeed'] else '❌ Failed'}"
    )
    ...
    simulation = Transactions.simulate_dag_transfer(
        source=alice.address, destination=bob.address, amount=100000000, network=network
    )
    ...
    if simulation["will_succeed"]:
        ...
        transaction = Transactions.create_dag_transfer(
            source=alice.address, destination=bob.address, amount=100000000
        )
