...
from constellation_sdk import (
    Account, Network, MetagraphClient,
...
def demo_discovery_capabilities():
    ...
    for network, data in summary['networks'].items():
        print(f"   🌐 {network.upper()}: {data['estimated_production']} production, "
...
def demo_advanced_features():
    ...
    print(f"   Filtered out: {len(all_deployments) - len(production_only)} test deployments")
    ...
    for network in networks:
        try:
            ...
            print(f"   {network}: {summary['production_count']} production, "
