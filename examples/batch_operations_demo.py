...
def main():
    ...
    operations = [
        create_batch_operation('get_balance', {'address': alice.address}, 'alice_balance'),
        create_batch_operation('get_ordinal', {'address': alice.address}, 'alice_ordinal'),
        create_batch_operation('get_transactions', {'address': alice.address, 'limit': 5}, 'alice_txs'),
    ...
    try:
        ...
        print(f"   Individual calls: {individual_time:.3f}s ({len(addresses)} requests)")
    ...
    try:
        ...
        if overview['transactions']:
            ...
            for i, tx in enumerate(overview['transactions'][:3], 1):
                ...
                print(f"      {i}. {tx.get('hash', 'N/A')[:16]}... ({amount / 1e8:.8f} DAG)")
    ...
    try:
        ...
        mixed_operations = [
            create_batch_operation('get_balance', {'address': alice.address}, 'valid_balance'),
            create_batch_operation('get_balance', {'address': 'INVALID_ADDRESS'}, 'invalid_balance'),
            create_batch_operation('get_ordinal', {'address': bob.address}, 'valid_ordinal'),
            create_batch_operation('get_transactions', {'address': 'DAG' + 'x' * 35}, 'invalid_txs'),
        ]
    ...
    if ASYNC_AVAILABLE:
        ...
        async def async_batch_demo():
            ...
            try:
                ...
                print(f"   Concurrent execution: {response.summary.get('concurrent_execution', False)}")
                print(f"   Success rate: {response.success_rate():.1f}%")
                print()
    ...
    try:
        ...
        for i, address in enumerate(addresses):
            portfolio_operations.append(
                create_batch_operation('get_balance', {'address': address}, f'portfolio_balance_{i}')
            )
        
        # Add transaction history for active addresses
        for i, address in enumerate(addresses[:2]):
            portfolio_operations.append(
                create_batch_operation('get_transactions', {'address': address, 'limit': 10}, f'portfolio_txs_{i}')
            )
        ...
        print(f"📊 Executing portfolio analysis ({len(portfolio_operations)} operations)...")
